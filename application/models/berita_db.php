...
parent::__construct();
    ...
    public function tambah_x($id='',$data) {
        ...
        $data['isi'] = $this->dbx->rows($sql);
				...
				if ($data['isi']== NULL ) {
        	unset($data['isi']);
	        $sql="SELECT ".$this->dbx->tablecolumn('hrm_berita').",1 as aktif,CURRENT_DATE() as tanggal";
					//echo $sql;die;
        	$data['isi']=$this->dbx->rows($sql);
        }
    ...
    public function tambah($data) {
    	...
    	$this->db->trans_start();
        $this->db->insert('hrm_berita', $data);
	...
	public function ubah($data,$id) {
		...
		$this->db->update('hrm_berita', $data);
    ...
    public function tambah_tujuan($id='',$data) {
        ...
        $data['idrole_opt'] = $this->dbx->data("SELECT j.replid,j.role as nama
        											,(SELECT '1' FROM hrm_berita_tujuan WHERE berita_id='".$id."' AND idrole=j.replid) as checked
        											FROM role j
													WHERE j.aktif=1
													ORDER BY j.role");
        return $data;
    ...
    public function hapus_tujuan_p_db($id) {
	    ...
	    $this->db->delete('hrm_berita_tujuan');
	    if ($this->db->_error_number() == 0) {
	    	return true;
	    } else {
	        return false;
    ...
    public function tambah_tujuan_p_db($data) {
    	//echo print_r(array_values($data));die;
    	$this->db->trans_start();
        $this->db->insert('hrm_berita_tujuan', $data);
        $insert_id = $this->db->insert_id();
        if ($this->db->affected_rows() > 0) {
               $this->db->trans_complete();
               return true;
