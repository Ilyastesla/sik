...
Class ksw_kelas_db extends CI_Model {
    ...
    public function tambah_db($id='',$data) {
		...
		$data['kelompok_siswa_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',departemen,'] ',kelompok) as nama FROM kelompoksiswa ks WHERE ks.departemen IN (".$this->dbx->sessionjenjangtext().") AND ks.aktif=1 ORDER BY ks.departemen,ks.kelompok",'none');
