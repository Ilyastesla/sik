...
Class ns_rapot_db extends CI_Model {
    ...
    public function tambah_db($id='',$data) {
        ...
        $data['idtahunajaran_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',departemen,'] ',tahunajaran) as nama FROM tahunajaran WHERE  idcompany='".$this->session->userdata('idcompany')."' AND idcompany='".$this->input->post('idcompany')."' AND (aktif=1 OR replid='".$data['isi']->idtahunajaran."') and departemen IN (SELECT departemen FROM departemen WHERE replid IN (".$this->session->userdata('dept').")) ORDER BY aktif DESC ,nama DESC ",'up');
        ...
        $data['idrapottipe_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',iddepartemen,'] ',rapottipe,' ',keterangan, ' (',IF(aktif=1,'A','T'),')') as nama FROM ns_rapottipe WHERE k13<>1 AND aktif=1 AND iddepartemen IN (SELECT departemen FROM tahunajaran  WHERE replid='".$data['isi']->idtahunajaran."') ORDER BY nama ",'up');

        $data['idperiode_opt'] = $this->dbx->opt("SELECT replid,periode as nama FROM ns_periode ORDER BY nama");
		$data['idtahunajaranrapot_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',departemen,'] ',tahunajaran) as nama FROM tahunajaran WHERE idcompany='".$this->input->post('idcompany')."' AND departemen IN (SELECT departemen FROM tahunajaran  WHERE replid='".$data['isi']->idtahunajaran."') ORDER BY aktif DESC ,nama DESC" ,'up');
        return $data;
  }
	...
	public function view_db($id,$data) {
      	$sql="SELECT pv.*,YEAR(pv.tanggalkegiatan) as tahunkegiatan,ta.tahunajaran,tar.tahunajaran as tahunajaranrapot
      			...
      			,s.replid as replidsiswa,s.tmplahir,s.tgllahir,s.nomorpeserta
						 ...
						 ,rt.batasnilai,com.nama as companytext,com.city as citytext,com.logo as logotext,com.cap as captext
      			FROM ns_rapot pv
      			LEFT JOIN tahunajaran ta ON ta.replid=pv.idtahunajaran
		    	...
		    	if ($nilaimurni<>1){
			    		//$sql="SELECT COUNT(pj.replid) as jml,SUM(pdn.nilai) as nilaitot,rmpd.nilai as rmpdnilai,rmv.nilai as rmvnilai ,rm.persentase
							$sql="SELECT AVG(pdn.nilai) as nilaitot,rmpd.nilai as rmpdnilai,rmv.nilai as rmvnilai ,rm.persentase
										FROM ns_pembelajaranjadwal pj
										INNER JOIN ns_pembelajaranjadwalrapottipe pjrt ON pj.replid=pjrt.idpembelajaranjadwal
										INNER JOIN (SELECT * FROM ns_pengembangandirinilai
										WHERE terdaftar = 1
										AND idregion = '".$idregion."'
										AND idsiswa = '".$idsiswa."') pdn ON pdn.idpembelajaranjadwal=pj.replid
										INNER JOIN ns_pengembangandirivariabel pdv ON pdv.replid=pdn.idpengembangandirivariabel
