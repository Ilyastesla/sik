...
Class approval_ppkb_db extends CI_Model {
...
parent::__construct();
    ...
    public function view_db($id='',$data) {
        ...
        $sql="SELECT pm.*,u.unit as idunit FROM hrm_ppkb_lain pm
        		INNER JOIN inventory_unit u ON pm.idunit=u.replid
        		WHERE idppkb='".$id."'";
        ...
        $next_node=$this->dbx->next_node($data['isi']->status,"ppkb");
        ...
        $sqlapprover="SELECT DISTINCT p.replid,CONCAT(p.nama,' (',jx.jabatan,')') as nama
        				...
        				WHERE p.aktif=1
        					...
        					AND pj.idjabatan IN (SELECT j.replid FROM hrm_loa l
        										...
        										WHERE idmodul='PPKB' AND l.node='".$next_node."'
