...
Class ksw_prestasi_pd_db extends CI_Model {
    ...
    public function tambah_db($data,$id='') {
       ...
       if ($data['isi']== NULL ) {
            unset($data['isi']);
            $sql="SELECT ".$this->dbx->tablecolumn('ksw_prestasi_pd').",current_date() as tanggallaporan, current_date() as tanggalkonseling,0 as idtahunajaran,0 as iddepartemen,0 as idcompany ";
            $data['isi']=$this->dbx->rows($sql);
       }
     ...
     public function view_db($data,$id,$idx="") {
        $sql = "SELECT r.*,s.nis,s.nama as namasiswatext,s.abk,s.aktif as aktifsiswa
                        ...
                        ,jl.nama as jenislaporantext,t.nama as tempattext,rp.prioritas as prioritastext
                        ,st.status as statustext
                        ,(SELECT 1 FROM ksw_prestasi_pdreport WHERE idkonseling=r.replid AND fase=1 LIMIT 1) as onproses
                FROM ksw_prestasi_pd r
                LEFT JOIN siswa s ON s.replid=r.idsiswa
                LEFT JOIN kelas k ON k.replid=r.idkelas
                LEFT JOIN pegawai p ON p.replid=k.idwali 
        ...
        $sql="SELECT * FROM ksw_prestasi_pdreport WHERE replid='".$idx."' ORDER BY created_date LIMIT 1";
        ...
        if ($data['isi2']== NULL ) {
                unset($data['isi2']);
                $sql="SELECT ".$this->dbx->tablecolumn('ksw_prestasi_pdreport').",current_date() as tanggalkonseling ";
                $data['isi2']=$this->dbx->rows($sql);
        }
	return $data;
