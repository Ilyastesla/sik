...
Class user_authentication_db extends CI_Model {
  ...
  // Read data using  username and password
    ...
    public function read_user_information($sess_array) {
		...
		public function readcompany($idcompanylogin,$companyuser) {
			if($companyuser<>""){
				$filtercompany= " AND replid IN (".$companyuser.")";
			}else{
				$filtercompany=" AND replid is NULL ";
			}
      	$sql = "SELECT * FROM hrm_company WHERE replid='".$idcompanylogin."' ".$filtercompany;
				$query=$this->dbx->rows($sql);
        if ($query<>null) {
            return $query;
        } else {
