...
Class ksw_pindahkelas_db extends CI_Model {
    ...
    public function data() {
      ...
      /*
			$data['idtahunajarantujuan_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',departemen,'] ',tahunajaran) as nama FROM tahunajaran
																													WHERE departemen='".$this->input->post('iddepartemen')."' AND tglmulai > (SELECT tglmulai FROM tahunajaran WHERE replid='".$this->input->post('idtahunajaran')."')
																													ORDER BY aktif DESC ,nama DESC ",'up');
      $data['idtingkattujuan_opt'] = $this->dbx->opt("SELECT replid,tingkat as nama FROM tingkat WHERE aktif=1 AND departemen='".$this->input->post('iddepartemen')."'  AND tingkat=(SELECT tingkat FROM tingkat WHERE replid='".$this->input->post('idtingkat')."') ORDER BY CAST(tingkat AS SIGNED) ASC",'up');
      */
		...
		$data['idkelas_opt'] = $this->dbx->opt("SELECT k.replid,k.kelas as nama FROM kelas k
																								...
																								ORDER BY nama",'up');
		//AND k.replid<>'".$this->input->post('idkelas')."'
