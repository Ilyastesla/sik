...
Class realisasi_ppkb_cek_db extends CI_Model {
...
parent::__construct();
    ...
    public function view_db($id='',$data) {
        ...
        //attachment
        $sql2="SELECT * FROM hrm_ppkb_cek_attachment WHERE idppkb='".$id."'";
    ...
    public function kode_transaksi($company,$tanggalpengajuan){
	    ...
	    $sql="SELECT CONCAT(company_code,'/realisasi_ppkb_cek/',(SELECT DATE_FORMAT('".$tanggalpengajuan."','%Y%m')),'/') as kode_transaksi
	    ...
	    $sql2="SELECT LPAD(RIGHT(RIGHT(trim(kode_transaksi),11),4)+1,4,'0') as kode_transaksi2 FROM hrm_ppkb_cek_realisasi WHERE idcompany='".$company."'
 and LEFT(RIGHT(trim(kode_transaksi),11),6)=DATE_FORMAT('".$tanggalpengajuan."','%Y%m') ORDER BY kode_transaksi2 DESC LIMIT 1";
    ...
    public function ubahlain_x($id='',$data) {
        ...
        if ($data['isi']->keterangan_realisasi <> "" ) {
        	unset($data['isi']);
        	$sql="SELECT
	        			keterangan_realisasi as keterangan,
						jumlah_realisasi as jumlah,
						idunit_realisasi as idunit,
						nilai_realisasi as nilai
						,idkredit
    ...
    public function adjustmentlain($id) {
    	$sql="SELECT SUM(sub_total) as total,idadjustment FROM hrm_ppkb_cek_lain WHERE idppkb='".$id."' GROUP BY idadjustment";
