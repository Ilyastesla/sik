...
Class ksw_kenaikan_db extends CI_Model {
    ...
    public function data() {
			...
			$data['idtahunajarantujuan_opt'] = $this->dbx->opt("SELECT replid,CONCAT('[',departemen,'] ',tahunajaran) as nama FROM tahunajaran
																													WHERE idcompany='".$this->input->post('idcompany')."' AND departemen='".$this->input->post('iddepartemen')."' AND tglmulai > (SELECT tglmulai FROM tahunajaran WHERE replid='".$this->input->post('idtahunajaran')."')
			...
			$data['idtingkattujuan_opt'] = $this->dbx->opt("SELECT replid,tingkat as nama FROM tingkat
																											WHERE aktif=1 AND departemen='".$this->input->post('iddepartemen')."'  AND urutan > (SELECT urutan FROM tingkat WHERE replid='".$this->input->post('idtingkat')."') ORDER BY CAST(tingkat AS SIGNED) ASC",'up');
