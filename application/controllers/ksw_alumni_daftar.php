...
Class ksw_alumni_daftar extends CI_Controller {
...
parent::__construct();
		...
		// Load database
		$this->load->model('ksw_alumni_daftar_db');

		if( $this->session->userdata('logged_in')) {
			if($this->dbx->checkpage($this->session->userdata('role_id'),'ksw_alumni_daftar')==false){
	...
	{
			$data= $this->ksw_alumni_daftar_db->data();
			$data['form']='Daftar Alumni';
			$data['view']='index';
			$data['action']='ksw_alumni_daftar';
			$data['actionsave']='ksw_alumni_daftar/tambah_p';
			$this->load->view('ksw_alumni_daftar_v', $data);
	}
