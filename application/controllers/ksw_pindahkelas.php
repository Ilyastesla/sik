...
Class ksw_pindahkelas extends CI_Controller {
...
parent::__construct();
		...
		// Load database
		$this->load->model('ksw_pindahkelas_db');

		if( $this->session->userdata('logged_in')) {
			if($this->dbx->checkpage($this->session->userdata('role_id'),'ksw_pindahkelas')==false){
	...
	{
			$data= $this->ksw_pindahkelas_db->data();
			$data['form']='Pindah Kelas';
			$data['view']='index';
			$data['action']='ksw_pindahkelas';
			$data['actionsave']='ksw_pindahkelas/tambah_p';
			$this->load->view('ksw_pindahkelas_v', $data);
	}
	...
	public function tambah_p() {
			...
			foreach ($idsiswarow as $idsiswa) {
					...
					$data = array(
													...
													"aktif"=>1,
													"keterangan"=>"Pindah Kelas Atau Regional",
													"modified_date"=> $this->dbx->cts(),
									 				"modified_by"=> $this->session->userdata('idpegawai')
												);
