...
Class keu_administrasi_calon_komo extends CI_Controller {
...
parent::__construct();
		...
		if( $this->session->userdata('logged_in')) {
			if($this->dbx->checkpage($this->session->userdata('role_id'),'keu_administrasi_calon_komo')==false){
	...
	public function index($print="",$excel="")
	{
			...
			$data['view']='index';
			$data['action']='keu_administrasi_calon_komo';
			$data['excel']=$excel;
			$data['hanyapusat']=1;
	...
	public function set_keucalon($keutype,$idcalon) {
		...
		if ($result == TRUE) {
			//redirect('keu_administrasi_calon_komo');
			?><script>
