...
Class realisasi_ppkb_cek extends CI_Controller {
...
public function __construct() {
	parent::__construct();
		...
		// Load database
		$this->load->model('realisasi_ppkb_cek_db');

   if( $this->session->userdata('logged_in')) {
       if($this->dbx->checkpage($this->session->userdata('role_id'),'realisasi_ppkb_cek')==false){
	...
	{
		$data['show_table'] = $this->realisasi_ppkb_cek_db->data();
		$data['form']='Laporan Pertanggungjawaban PPKB';
		$data['view']='index';
		$this->load->view('realisasi_ppkb_cek_v', $data);
	}
	//VIEW
	//-------------------------------------------------------------------------------------------
	...
	public function view($id,$stat='') {
		...
		$data['view']='bayar';
		$data['view2']=1;
		$data['action']='realisasi_ppkb_cek/approve_p/'.$id;
		$data['idx']=$id;
		$data= $this->realisasi_ppkb_cek_db->view_db($id,$data);
		$this->load->view('realisasi_ppkb_cek_v',$data);
	}
	...
	public function bayar($id,$stat='') {
		...
		$data['view']='bayar';
		$data['view2']=0;
		$data['action']='realisasi_ppkb_cek/bayar_p/'.$id;
		$data['idx']=$id;
		$data= $this->realisasi_ppkb_cek_db->view_db($id,$data);
		$this->load->view('realisasi_ppkb_cek_v',$data);
	}
	...
	public function tambahkeperluan($id,$idx="",$data="",$stat="") {
		...
		$data['action']='realisasi_ppkb_cek/tambahkeperluan_p/'.$id.'/'.$idx;
		$data['idx']=$id;
		$data= $this->realisasi_ppkb_cek_db->ubahkeperluan_x($idx,$data);
		$this->load->view('realisasi_ppkb_cek_v',$data);
	}
	...
	public function tambahkeperluan_p($id,$idx='') {
		...
		if ($idx<>""){
			$result = $this->realisasi_ppkb_cek_db->ubahkeperluan_db($data,$idx) ;
		}else{
			$idx = $this->realisasi_ppkb_cek_db->tambahkeperluan_db($data);
			if ($idx<>""){$result=TRUE;}
		}
		if ($result == TRUE) {
	...
	public function tambahjasa($id,$idx="",$data="",$stat="") {
		...
		$data['action']='realisasi_ppkb_cek/tambahjasa_p/'.$id.'/'.$idx;
		$data['idx']=$id;
		$data= $this->realisasi_ppkb_cek_db->ubahjasa_x($idx,$data);
		$this->load->view('realisasi_ppkb_cek_v',$data);
	}
	...
	public function tambahjasa_p($id,$idx='') {
		...
		if ($idx<>""){
			$result = $this->realisasi_ppkb_cek_db->ubahjasa_db($data,$idx) ;
		}else{
			$idx = $this->realisasi_ppkb_cek_db->tambahjasa_db($data);
			if ($idx<>""){$result=TRUE;}
		}
		if ($result == TRUE) {
			redirect('realisasi_ppkb_cek/bayar/'.$id);
	...
	public function tambahlain($id,$idx="",$data="",$stat="") {
		...
		$data['action']='realisasi_ppkb_cek/tambahlain_p/'.$id.'/'.$idx;
		$data['idx']=$id;
		$data= $this->realisasi_ppkb_cek_db->ubahlain_x($idx,$data);
		$this->load->view('realisasi_ppkb_cek_v',$data);
	}
	...
	public function tambahlain_p($id,$idx='') {
		...
		if ($idx<>""){
			$result = $this->realisasi_ppkb_cek_db->ubahlain_db($data,$idx) ;
		}else{
			$idx = $this->realisasi_ppkb_cek_db->tambahlain_db($data);
			if ($idx<>""){$result=TRUE;}
		}
		if ($result == TRUE) {
			redirect('realisasi_ppkb_cek/bayar/'.$id);
	...
	public function bayar_p($id='') {
		if ($this->input->post("status")==4){
			$this->realisasi_ppkb_cek_db->adjustmentlain($id);
		}
	  	$data = array(
		...
		$result = $this->realisasi_ppkb_cek_db->ubah($data,$id) ;
		if ($result == TRUE) {
			redirect('realisasi_ppkb_cek/view/'.$id);
		} else {
			$data['error']='Errorr...';
			redirect('/realisasi_ppkb_cek/bayar/'.$id);
		}
