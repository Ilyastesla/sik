...
function chatWith(chatuser, chatuserName,focustext) {
	chat_with = true;
...
function createChatBox(chatboxtitle,chatuserName, minimizeChatBox) {
	if ($("#chatbox_"+chatboxtitle).length > 0) {
		...
		.addClass("chatbox")
		.html('<div class="chatboxhead"><div class="chatboxtitle">'+getTitle( chatuserName )+'</div><div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">-</a> <a href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">X</a></div><br clear="all"/></div><div class="chatboxcontent"></div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\',\''+chatuserName+'\');" onfocus="javascript:return readchat(event,this,\''+chatboxtitle+'\',\''+chatuserName+'\');" ></textarea></div>')
		.appendTo($( "body" ));
	...
	//call
	$.ajax({
	  url: site+"chat?action=chathistory",
	  cache: false,
...
function readchat(event,chatboxtextarea,chatboxtitle, boxUserName) {
			$.post("chat?action=readchat", {to: chatboxtitle});
}
...
function chatHeartbeat(){
	...
	$.ajax({
	  url: site+"chat?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			$.post("chat?action=sendchat", {to: chatboxtitle, to_name: boxUserName, message: message} , function(data){
				...
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+getTitle("Saya")+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+message+'</span></div>');
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop($("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
			});
		}
		chatHeartbeatTime = minChatHeartbeat;
...
function startChatSession(){
	$.ajax({
	  url: site+"chat?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
